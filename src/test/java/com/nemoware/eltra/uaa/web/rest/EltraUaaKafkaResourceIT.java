...
import com.nemoware.eltra.uaa.config.KafkaProperties;
...
class EltraUaaKafkaResourceIT {
    ...
    void setup() {
        ...
        EltraUaaKafkaResource kafkaResource = new EltraUaaKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/eltra-uaa-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/eltra-uaa-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
